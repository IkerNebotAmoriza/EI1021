...
public class Partida {
	...
	public void iniciaDisparos(int size) {
		misDisparos = new boolean[size][size];
		for (int row = 0; row < size; row++) {
			for (int col = 0; col < size; col++) {
				misDisparos[row][col] = false;
			}
		}
	}
	
	public boolean casillaDisparada(int row, int col) {
		return misDisparos[row][col];
	}
    ...
    public int pruebaCasilla(int f, int c) {
    		...
    		if (mar[f][c] >= 0) {
    			...
    			if (barco.getTocadas() == barco.getTamanyo()) {
    				int i = barco.getFilaInicial();
    	        		int j = barco.getColumnaInicial();
    	        		for(int z = 0; z < barco.getTamanyo(); z++){
    	        			mar[i][j] = HUNDIDO;
    	        			if(barco.getOrientacion()=='H') j++;
    	        			else i++;
    	        		}
    	        		quedan--;
    	        		return idBarco;
	...
	public String getBarco(int idBarco) {
		Barco barco = barcos.get(idBarco);		
		return barco.getFilaInicial() + "#" + barco.getColumnaInicial() + "#" + barco.getOrientacion() + "#" + barco.getTamanyo();
	}
	...
	public String[] getSolucion() {
		String [] datosBarcos = new String[barcos.size()];
        for(int i = 0; i < datosBarcos.length; i++)
        	datosBarcos[i] = getBarco(i);
		return datosBarcos;
	}
